...
Deno.serve(async (request) => {
  ...
  const { data: machine, error: findError } = await supabase.from('machines')
    .select('id, status, qr_token').eq('id', machine_id).maybeSingle();
  ...
  if (machine.status === 'in_use') return json({ error: 'Machine is already in use' }, 409);

  const { data: updated, error } = await supabase.from('machines')
    .update({ status: 'in_use' }).eq('id', machine_id).eq('qr_token', qr_token)
    .eq('status', 'free').select('id, in_use_since').maybeSingle();
  if (error) {
    console.error('Machine update failed', error);
    return json({ error: 'Unable to update machine' }, 500);
  }
  if (!updated) return json({ error: 'Machine is already in use' }, 409);
  return json({
    success: true,
    machine_id: updated.id,
    in_use_since: updated.in_use_since,
    estimated_done_time: new Date(new Date(updated.in_use_since).getTime() + 50 * 60 * 1000).toISOString(),
  });
});
